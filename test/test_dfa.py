...
class MyTestCase(unittest.TestCase):
    def test_dfa_match_0(self):
        # (a|b)*abb
        nodes = [DFA.State(0), DFA.State(1), DFA.State(2), DFA.State(3, True)]
        dfa_tab = (
            {'a': 1, 'b': 0},
            {'a': 1, 'b': 2},
            {'a': 1, 'b': 3},
        ...
        s0 = nodes[0]
        nodes[3].is_end = True
        print(FiniteAutomaton.get_dot_content(s0))
        ...
        for test_case in test_cases:
            self.assertEqual(DFA.dfa_match(s0, test_case[0]), test_case[1])
    ...
    def test_dfa_match_1(self):
        ...
        nodes = [DFA.State('s0'), DFA.State('s1'),
                 DFA.State('s2'), DFA.State('s3', True)]
        dfa_tab = (
            {'a': 1, 'b': 0},
            {'a': 1, 'b': 2},
            {'a': 1, 'b': 3},
